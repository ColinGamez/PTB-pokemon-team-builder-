...
class BreedingGoal:
    ...
    perfect_iv_priority: List[str] = None  # Priority order for perfect IVs
    minimum_iv_requirements: Dict[str, int] = None
    ...
    def evaluate_fitness(self, pokemon: BreedingPokemon) -> float:
        ...
        for stat in ["hp", "attack", "defense", "sp_attack", "sp_defense", "speed"]:
            ...
            # Priority stat bonus
            if stat in self.perfect_iv_priority[:3] and actual_iv == 31:
                iv_score += 5
            
            # Minimum requirement check
            min_req = self.minimum_iv_requirements.get(stat, 0)
